...
# start the measure
class HVACTemplateZoneVAVFanPowered < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "HVACTemplate:Zone:VAV:FanPowered"
  end

  # human readable description
  def description
    return "Create HVACTemplate:Zone:VAV:FanPowered"
  end
...
#populate choice argument for thermal zones in the model
	...
	#Flow type
	flow_type = OpenStudio::StringVector.new
	flow_type << "Parallel"
	flow_type << "Series"
	flow_type = OpenStudio::Ruleset::OSArgument::makeChoiceArgument('flow_type', flow_type, true)
	flow_type.setDisplayName("Choose flow type.")
    flow_type.setDefaultValue("Parallel")
    args << flow_type
    ...
    reheat_type.setDefaultValue("Electric")
    args << reheat_type
...
# assign the user inputs to variables
	...
	flow_type = runner.getStringArgumentValue("flow_type",user_arguments)
   ...
   # Hook the VAV system to each zone
	...
	#terminal_fan
	terminal_fan = OpenStudio::Model::FanConstantVolume.new(model, system_sch)
    terminal_fan.setName("#{zone.name} Term Fan")
    terminal_fan.setPressureRise(300)

    # VAV terminal
	if flow_type == 'Parallel'
	  terminal = OpenStudio::Model::AirTerminalSingleDuctParallelPIUReheat.new(model, system_sch, terminal_fan, rht_coil)
      terminal.setName("#{zone.name} PFP Term")
      selected_airloop.addBranchForZone(zone, terminal.to_StraightComponent)
    elsif flow_type == 'Series'
	  terminal = OpenStudio::Model::AirTerminalSingleDuctSeriesPIUReheat.new(model, terminal_fan, rht_coil)
      terminal.setName("#{zone.name} SFP Term")
      selected_airloop.addBranchForZone(zone, terminal.to_StraightComponent)
    end
...
HVACTemplateZoneVAVFanPowered.new.registerWithApplication
