...
# start the measure
class HVACTemplateZoneVAVHeatAndCool < OpenStudio::Ruleset::ModelUserScript

  # human readable name
  def name
    return "HVACTemplate:Zone:VAV:HeatAndCool"
  end

  # human readable description
  def description
    return "Create HVACTemplate:Zone:VAV:HeatAndCool"
  end
   ...
   # Hook the VAV system to each zone
    ...
    # Reheat coil
      ...
      if reheat_type == "None"
	     ...
	     terminal = OpenStudio::Model::AirTerminalSingleDuctVAVHeatAndCoolReheat.new(model,rht_coil)
         terminal.setName("#{zone.name} VAV Term")
         selected_airloop.addBranchForZone(zone, terminal.to_StraightComponent)
      end
...
HVACTemplateZoneVAVHeatAndCool.new.registerWithApplication
